                ...
                <div class="p-6 text-gray-900">
                    ...
                    <form method="POST" action="{{ route('admin.events.update', $event) }}" class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="event_name" value="{{ old('event_name', $event->event_name) }}" class="mt-1 w-full border-gray-300 rounded-md shadow-sm">
                            @error('event_name')<p class="text-sm text-red-600 mt-1">{{ $message }}</p>@enderror
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700">Deskripsi</label>
                            <textarea name="event_description" rows="4" class="mt-1 w-full border-gray-300 rounded-md shadow-sm">{{ old('event_description', $event->event_description) }}</textarea>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="datetime-local" name="event_date" value="{{ old('event_date', \Illuminate\Support\Carbon::parse($event->event_date)->format('Y-m-d\TH:i')) }}" class="mt-1 w-full border-gray-300 rounded-md shadow-sm">
                                @error('event_date')<p class="text-sm text-red-600 mt-1">{{ $message }}</p>@enderror
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Kapasitas</label>
                                <input type="number" name="event_capacity" value="{{ old('event_capacity', $event->event_capacity) }}" class="mt-1 w-full border-gray-300 rounded-md shadow-sm">
                        ...
                        <div>
                            ...
                            <input type="text" name="event_location" value="{{ old('event_location', $event->event_location) }}" class="mt-1 w-full border-gray-300 rounded-md shadow-sm">
                        ...
                        <div>
                            ...
                            <input list="creator-list" name="events_creators_id" value="{{ old('events_creators_id', $event->events_creators_id) }}" class="mt-1 w-full border-gray-300 rounded-md shadow-sm" placeholder="Pilih creator">
