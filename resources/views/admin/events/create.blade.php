                    ...
                    <form method="POST" action="{{ route('admin.events.store') }}" class="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea name="event_description" rows="4"
                                class="mt-1 w-full border-gray-300 rounded-md shadow-sm">{{ old('event_description') }}</textarea>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="number" name="event_capacity" value="{{ old('event_capacity') }}"
                                    class="mt-1 w-full border-gray-300 rounded-md shadow-sm">
                        ...
                        <div>
                            ...
                            <input list="creator-list" name="events_creators_id" value="{{ old('events_creators_id') }}"
                                class="mt-1 w-full border-gray-300 rounded-md shadow-sm" placeholder="Pilih creator">
                            <datalist id="creator-list">
                                @foreach($creators as $creator)
                                    <option value="{{ $creator->id }}">
                                        {{ $creator->user->name ?? 'User #' . $creator->user_id }}
                                        ({{ $creator->user->email ?? '' }})
                                    </option>
                                @endforeach
                            </datalist>
                            @error('events_creators_id')<p class="text-sm text-red-600 mt-1">{{ $message }}</p>@enderror
                            <p class="text-xs text-gray-500 mt-1">Ketik nama/email untuk rekomendasi, nilai yang
                                disimpan adalah ID creator.</p>
                        </div>
