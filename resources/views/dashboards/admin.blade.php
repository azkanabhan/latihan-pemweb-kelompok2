    ...
    @if(isset($admin_requested_events) && $admin_requested_events->count())
        <div class="space-y-4">
            ...
            @foreach($admin_requested_events as $ev)
                <div
                    class="border-2 border-gray-200 hover:border-indigo-400 p-5 rounded-xl bg-white hover:shadow-lg transition-all duration-200 group flex justify-between items-start gap-4">

                    {{-- 2. Bagian info (kiri) dibungkus dengan <a> agar tetap bisa diklik --}}
                        <a href="{{ route('admin.events.show', $ev) }}" class="flex-1 min-w-0">
                            <div class="flex items-center gap-3 mb-2">
                                <h3 class="text-lg font-bold text-gray-900 group-hover:text-indigo-700 transition-colors">
                                    {{ $ev->event_name }}
                                </h3>
                                <span
                                    class="px-2 py-1 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800 border border-yellow-200 flex-shrink-0">
                                    Requested
                                </span>
                            </div>
                            <div class="flex flex-wrap items-center gap-3 text-sm text-gray-600 mb-2">
                                ...
                                <span class="flex items-center gap-1">
                                    ...
                                    {{ $ev->start_date?->format('d M Y, H:i') }}
                                </span>
                            </div>
                            ...
                            <div
                                class="mt-3 text-xs text-indigo-600 font-medium flex items-center gap-1 group-hover:gap-2 transition-all">
                        ...
                        {{-- 3. Bagian tombol (kanan) diletakkan di luar <a> --}}
                            {{-- 4. 'onclick' yang bermasalah DIHAPUS dari div ini --}}
